...
def interface(ctx: Dict, debug: bool) -> None:
    ...
    ctx.obj["DB_PATH"] = os.path.join(os.getcwd(), "loanEventRepository.sqlite3")
    if debug:
...
def create_db(ctx: Dict) -> None:
    ...
    with sqlite3.connect(ctx.obj["DB_PATH"]) as connection:
        if not connection:
            click.echo(
                "Error: Unable to create sqlite3 loanEventRepository file. Please ensure sqlite3 is installed on your system and "
                "available in PATH!"
            )
            return
...
def load(ctx: Dict, filename: str) -> None:
    ...
    if not os.path.exists(ctx.obj["DB_PATH"]):
        click.echo(f"Database does not exist at {ctx.obj['DB_PATH']}, please create it using `create-loanEventRepository` command")
        return
