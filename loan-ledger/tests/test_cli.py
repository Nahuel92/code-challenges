...
class CLITest(unittest.TestCase):
    ...
    def test_create_db(self):
        ...
        with self.runner.isolated_filesystem(temp_dir="/tmp"):
            result = self.runner.invoke(interface, ["create-loanEventRepository"])
            self.assertEqual(0, result.exit_code)
            self.assertEqual(True, os.path.exists(os.path.join(os.getcwd(), "loanEventRepository.sqlite3")))

    def test_drop_db(self):
        """Test deletion of DB."""
        with self.runner.isolated_filesystem(temp_dir="/tmp"):
            self.runner.invoke(interface, ["create-loanEventRepository"])
            result = self.runner.invoke(interface, ["drop-loanEventRepository"])
            self.assertEqual(0, result.exit_code)
            self.assertEqual(False, os.path.exists(os.path.join(os.getcwd(), "loanEventRepository.sqlite3")))
    ...
    def test_load(self):
        ...
        with self.runner.isolated_filesystem(temp_dir="/tmp"):
            ...
            self.assertEqual(0, result.exit_code)
            self.assertEqual(
                f"Database does not exist at {os.path.join(os.getcwd(), 'loanEventRepository.sqlite3')},"
                f" please create it using `create-loanEventRepository` command\n",
                result.output,
            )
            self.runner.invoke(interface, ["create-loanEventRepository"])
            result = self.runner.invoke(interface, ["load", test_file_1])
            ...
            self.runner.invoke(interface, ["drop-loanEventRepository"])
            self.runner.invoke(interface, ["create-loanEventRepository"])
